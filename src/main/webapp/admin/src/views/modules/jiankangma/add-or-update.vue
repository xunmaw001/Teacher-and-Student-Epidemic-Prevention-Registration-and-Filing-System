      ...
      <el-row>
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.jiankangma" label="健康码" prop="jiankangma">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiankangma.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jiankangma/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiankangmaCrossAddOrUpdateFlag = false;
                  this.parent.search();
